...
set content [split [string trim [read $f]]]
set numbers ""
foreach l $content {
    struct::set add numbers $l
}
...
proc func {x} {
    ...
    foreach y $numbers {
        ...
        if {$y eq $z} {
            continue
        } elseif {[struct::set contains $numbers $z]} {
            if {[lsearch $pairs $pair] >= 0 || [lsearch $pairs $pairx] >= 0} {
                continue
            } else {
                lappend pairs $pair
            }
...
proc get_flag {res} {
    ...
    for {set i 0} { $i < [llength $res] } { incr i } {
        set chr [format %c [format "%d" [func [lindex $res $i]]]]
        lappend flag $chr
    }
    return [join $flag "" ]
}
set res {751741232  519127658   583555720   3491231752  3333111256  481365731   982100628   1001121327  3520999746
    ...
    puts    [format "The flag is %s" [get_flag $res]]
